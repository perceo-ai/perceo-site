        ...
        <section className="mt-14 grid gap-4 lg:grid-cols-3">
          {products.map((product) => (
            <article
              id={product.slug}
              key={product.slug}
              className="flex flex-col rounded-[8px] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-md transition hover:border-white/20"
            >
              ...
              <p className="mt-4 flex-1 text-base leading-7 text-zinc-300">{product.pitch}</p>

              {product.install ? (
                <div className="mt-6 overflow-hidden rounded-[6px] border border-white/10 bg-black/25">
                  <div className="border-b border-white/10 px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.14em] text-zinc-500">
